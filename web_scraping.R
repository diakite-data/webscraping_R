...
get_consumer_information__name <- function(url){
html = read_html(url)
    html %>% 
    # The relevant tag
    html_nodes('.consumer-information__name') %>%      
...
get_reviewer_names <- function(html){
  first_page %>% 
    html_nodes('.consumer-information__review-icon') %>% 
    html_text() %>% 
    str_trim() %>% 
    unlist()
}
...
urll <- paste0(url_wikipedia, url_filmographie)
...
data_html %>% html_nodes('.consumer-information__review-count > span')   %>%
  html_text()

get_review_count = function(url)
{
  data_html %>% html_nodes('.consumer-information__review-count > span')   %>%
       html_text()
}
